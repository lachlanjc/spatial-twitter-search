...
Bun.write("lib/db/raw/likes3.json", JSON.stringify(likes));
const prevLikes1 = await Bun.file("lib/db/raw/likes1.json")
  .text()
  .then((text) => JSON.parse(text));
const prevLikes2 = await Bun.file("lib/db/raw/likes2.json")
  .text()
  .then((text) => JSON.parse(text));
likes.push(...prevLikes1, ...prevLikes2);
